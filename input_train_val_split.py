...
def get_files(file_dir, ratio):
    ...
    for file in os.listdir(file_dir):
        name = file.split(sep='.')
        if name[0]=='land':
            land.append(file_dir + file)
            label_land.append(0)
        else:
            road.append(file_dir + file)
            label_road.append(1)
    print('There are %d land\nThere are %d road' %(len(land), len(road)))
    
    image_list = np.hstack((land, road))
    label_list = np.hstack((label_land, label_road))
